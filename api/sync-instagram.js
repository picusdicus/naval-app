 ...
 * cuerpo (o en `{posts: [...]}`).
 */
async function obtenerPosts(body) {
 ...
 * (instagram/<shortCode>.<ext>): re-ejecutar el webhook sobrescribe el mismo
 * blob en vez de acumular copias. Devuelve la URL o null (la imagen es
 * opcional: un fallo aquí no descarta el evento).
 */
async function subirImagen(shortCode, urlOrigen) {
  ...
  try {
    ...
    const { url } = await put(`instagram/${shortCode}.${extension}`, contenido, {
      access: 'public',
      contentType: tipo,
  ...
  try {
    ...
    for (const ev of validos) {
      try {
        ...
        const imagenUrl = await subirImagen(ev.shortCode, ev.imagenOrigen)
