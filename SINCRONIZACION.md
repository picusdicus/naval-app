...
**Corte por antigüedad**: cada run de Apify reenvía sus ~50 posts más recientes, incluidos los de runs anteriores. `esPostReciente()` (`api/_instagram.js`, `MAX_DIAS_ANTIGUEDAD = 30`) descarta antes de llamar a Claude los posts con `timestamp` de más de 30 días — ahorra tokens (ya no se re-analiza cada día lo mismo) y, de paso, evita re-intentar la subida de sus imágenes (ver **Presupuesto de Vercel Blob** en `CLAUDE.md`). Un post sin `timestamp` parseable no se descarta, para no romper el reproceso manual de posts de prueba.
...
- **Imagen**: la primera foto del post se sube a Vercel Blob en `instagram/<shortCode>.<ext>` (las URLs del CDN de Instagram caducan). Si falla —incluida una cuota de Blob agotada o el store pausado, `subirImagen` nunca lanza— el evento entra sin foto y el UPDATE conserva la imagen previa. **Cuota de Blob**: antes del bucle se consulta en una sola query qué `origen_externo_id` ya tienen `imagen_url`; si la hay, se reutiliza sin llamar a `subirImagen` (`imagenesReutilizadas` en el resumen) — solo se sube si el post es nuevo o si quedó a `NULL` (reintento). **Solo la primera**: a diferencia del webhook de noticias, los eventos se quedan con una sola foto a propósito (no hay galería en la ficha de un evento).
...
**Galería completa del carrusel**: a diferencia del webhook de eventos, aquí se sube el post entero — un post "Sidecar" puede traer varias fotos con información distinta cada una (el de instalaciones deportivas, una foto por instalación con su horario y dirección), y quedarse solo con la primera perdía ese contenido. `todasLasImagenes(post)` (`api/_instagram.js`) hace el mismo baile de campos entre actores que `primeraImagen`, pero devolviendo el array completo. Cada foto (hasta `MAX_IMAGENES_POST = 6`, bajado de 12 por cuota de Blob) se sube a Blob: la primera en `instagram-noticias/<shortCode>.<ext>` (nombre histórico, sin tocar) y el resto en `instagram-noticias/<shortCode>-<n>.<ext>` (`subirImagen(..., sufijo)`); las subidas de un mismo post se paralelizan, nunca entre posts. `imagen_url` sigue siendo solo la primera; `imagenes_url` (jsonb) guarda el array completo en orden — `NULL` en filas antiguas o posts de imagen única. **Cuota de Blob**: la galería se trata como un bloque — si la noticia ya existe con `imagen_url`, se reutilizan `imagen_url` + `imagenes_url` guardadas sin pedir ninguna foto a Instagram (`imagenesReutilizadas` en el resumen); solo se sube la galería entera si la noticia es nueva o si quedó a `NULL`. El bucle de actividades/documentos aplica el mismo criterio, a dos niveles: la foto del post (`imagenPost`, compartida por la fila única y las hijas) se reutiliza si CUALQUIER fila ya existente de ese `shortCode` tiene imagen, y cada foto de cartel del carrusel se reutiliza si ya existe una hermana con esa misma foto. **El emparejamiento de la foto es por `imagen_origen_id`** (media id/shortcode que Instagram asigna a cada hija de un Sidecar, columna de `actividades`), no por título: si el post se reedita y las fotos cambian de orden o de número, el título por sí solo puede confundir dos actividades con nombres parecidos ("Torneo de Pádel" / "Torneo de Pádel Infantil"), mientras que el id no cambia. Solo cae a título equivalente si no hay id en algún lado (actor de Apify sin ese campo, o fila anterior a este cambio) — ver **Presupuesto de Vercel Blob** en `CLAUDE.md`.
...
- **Una pausa de Blob no para la ingesta**: `subirImagen` atrapa cualquier error (cuota agotada, store pausado, red) y devuelve `null`; el evento/noticia/actividad se guarda igual con `imagen_url` a `NULL`. `npm run rellenar:imagenes` (manual, `scripts/rellenar-imagenes.mjs`, no en el cron) recupera después esas imágenes. Ver **Presupuesto de Vercel Blob** en `CLAUDE.md` — el límite (2 000 Advanced Requests/mes) es compartido con comercios, eventos del panel y destacados. **Orden a seguir tras una pausa** (el store se reactiva aparte, en el dashboard de Vercel — plan/facturación, fuera de este repo): (1) desplegar el código que consulta Neon antes de subir, (2) `npm run rellenar:imagenes -- --dry-run` para ver cuántas filas quedaron pendientes (no llama a Blob, corre aunque el store siga pausado), (3) `npm run rellenar:imagenes` (sin `--dry-run`) por tandas — respeta `--limite` para no golpear la cuota de golpe recién recuperada, repitiendo hasta vaciar el pendiente, (4) solo entonces reactivar la cadencia normal de Apify. Al revés (Apify antes del paso 1), el primer run se encuentra todas las filas de la pausa a la vez.
