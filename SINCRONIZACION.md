...
- **Una pausa de Blob no para la ingesta**: `subirImagen` atrapa cualquier error (cuota agotada, store pausado, red) y devuelve `null`; el evento/noticia/actividad se guarda igual con `imagen_url` a `NULL`. `npm run rellenar:imagenes` (manual, `scripts/rellenar-imagenes.mjs`, no en el cron) recupera después esas imágenes. Ver **Presupuesto de Vercel Blob** en `CLAUDE.md` — el límite (2 000 Advanced Requests/mes) es compartido con comercios, eventos del panel y destacados. **Orden de reactivación tras una pausa**: (1) desplegar el código que consulta Neon antes de subir, (2) `npm run rellenar:imagenes -- --dry-run` para ver cuántas filas quedaron pendientes, (3) solo entonces reactivar el store y la cadencia normal de Apify — al revés, el primer run se encuentra todas las filas de la pausa a la vez.
