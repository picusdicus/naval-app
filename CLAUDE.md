...
- **Backoffice**: the Destacados tab in `/admin` (`TablesDestacados.jsx` + `api/super/destacados.js`) lists everything with contratante, vigencia and estado; clicking a `pendiente` badge approves/activates (via the edit form first if the row has no duration yet — see **Duration** below), "Rechazar" cancels without passing through activo; the edit form sets orden/inicio+duración/imagen.
